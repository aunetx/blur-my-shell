...
function fixture(originalClip = null) {
    ...
    const cards = [0, 1, 2, 3].map(i => {
        ...
        if (originalClip) card.set_clip(...originalClip);
        return card;
    });
...
test('switching from hidden clip to rounded mask preserves the original clip', () => {
    const originalClip = [2, 3, 400, 90];
    const {cards, group, controller} = fixture(originalClip);
    assert.equal(cards[3].clip[3], 0);
    group.layout_manager.expansion = 0.5;
    controller.update_group_messages(group);
    assert.deepEqual(cards[3].clip, originalClip);
    assert.equal(controller.original_clips.has(cards[3]), false);
    controller.disable();
    for (const card of cards)
        assert.deepEqual(card.clip, originalClip);
});

test('mask uses logical target bounds instead of scaled texture pixels', () => {
    const {cards, controller} = fixture();
    const effect = cards[1].effects[0];
    assert.ok(effect instanceof ShaderEffect);
    assert.deepEqual(Object.keys(effect.constructor.default_params), []);
    effect.target = {x: 2.5, y: 3.25, logicalWidth: 400.5, logicalHeight: 90.25,
        width: 801, height: 181};
    effect.vfunc_paint_target();
    assert.equal(effect.uniforms.texture_x, 2.5);
    assert.equal(effect.uniforms.texture_y, 3.25);
    assert.equal(effect.uniforms.texture_width, 400.5);
    assert.equal(effect.uniforms.texture_height, 90.25);
    assert.equal(effect.painted, true);
    controller.disable();
});
