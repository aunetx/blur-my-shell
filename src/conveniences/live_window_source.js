...
export const LiveWindowGroupSource = class LiveWindowGroupSource {
    ...
    track_source_paint(actor) {
        if (!this.on_source_paint || this.paint_signals.has(actor))
            return;

        try {
            const effect = new LiveSourcePaintSignal();
            const id = effect.connect('source-paint', () => this.on_source_paint?.(actor));
            actor.add_effect(effect);
            this.paint_signals.set(actor, { effect, id });
        } catch (e) { }
    }
