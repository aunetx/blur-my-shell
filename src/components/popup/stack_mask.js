...
const StackMaskEffectClass = class StackMaskEffect extends utils.ShaderEffect {
    constructor() {
        super();
    ...
    vfunc_paint_target(node, context) {
        if (this.target) {
            // The shared backend exposes its exact logical offscreen bounds.
            const box = [this.target.x, this.target.y,
                this.target.logicalWidth, this.target.logicalHeight];
            const key = box.join(',');
