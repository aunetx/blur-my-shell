...
const DASH_STYLES = ["transparent-dash", "light-dash", "dark-dash"];
...
class DashInfos {
  ...
  remove_dash_blur(dash_not_already_destroyed = true) {
    ...
    this.dash_blur_connections_ids.forEach((id) => {
      if (id) this.dash_blur.disconnect(id);
    });
    this.dash_blur_connections_ids = [];
    if (this.dash_destroy_id) this.dash.disconnect(this.dash_destroy_id);
  ...
  remove_style() {
    ...
    DASH_STYLES.forEach((style) => this.dash.remove_style_class_name(style));
  }

  destroy_dash(dash_not_already_destroyed = true) {
    if (!dash_not_already_destroyed) this.bg_manager.backgroundActor = null;
  ...
  change_blur_type() {
    ...
    let [background, background_group, bg_manager, paint_signals] =
  ...
  update_size() {
    if (this.dash_blur.is_static) {
      let [x, y] = this.get_dash_position(
        this.dash_container,
...
export const DashBlur = class DashBlur extends Signals.EventEmitter {
  ...
  blur_existing_dashes() {
    ...
    Main.uiGroup
      ...
      .filter((child) => {
        return (
          child.get_name() === "dashtodockContainer" &&
  ...
  try_blur(dash_container) {
    ...
    if (
      !dash_box
        .get_children()
        .some((child) => child.get_name() === "bms-dash-backgroundgroup")
    ) {
  ...
  blur_dash_from(dash, dash_container) {
    ...
    this.connections.connect(dash, ["notify::width", "notify::height"], (_) =>
      this.update_size(),
    );
  ...
  connect_to_overview() {
    ...
    if (this.settings.dash_to_dock.UNBLUR_IN_OVERVIEW) {
      this.connections.connect(Main.overview, "showing", (_) => this.hide());
      this.connections.connect(Main.overview, "hidden", (_) => this.show());
    }
  }
