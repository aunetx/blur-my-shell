...
class DashInfos {
    ...
    destroy_dash(dash_not_already_destroyed = true) {
        ...
        // Safely remove background_group: verify it actually has the expected parent
        const bg_parent = this.background_group.get_parent();
        if (bg_parent)
            bg_parent.remove_child(this.background_group);
    ...
    update_size() {
        if (!this.dash_background || !this.dash_container)
            return;
        ...
        if (this.dash_blur.is_static) {
            ...
            let [dash_x, dash_y] = this.dash_background.get_transformed_position();
            let [bg_x, bg_y] = this.background_group.get_transformed_position();

            if (!isNaN(dash_x) && !isNaN(bg_x)) {
                this.background.x = -(bg_x - monitor.x);
                this.background.y = -(bg_y - monitor.y);
                ...
                // Guard against NaN from get_dash_position when style class doesn't match
                ...
                if (this.dash_container.get_style_class_name().includes("top") ||
                    this.dash_container.get_style_class_name().includes("bottom"))
                    this.background.set_clip(
                        x,
            ...
            // Dynamic blur: size and position the blur widget directly over the dash background pill
            const w = this.dash_background.width || this.dash.width;
            const h = this.dash_background.height || this.dash.height;
            if (!w || !h)
                return;
...
export const DashBlur = class DashBlur extends Signals.EventEmitter {
    ...
    blur_dash_from(dash, dash_container) {
        ...
        if (dash_container._slider) {
            this.connections.connect(
                dash_container._slider,
                'notify::allocation',
                _ => this.update_size()
            );
        }
        ...
        // Defer initial update_size until after layout pass to avoid
        // "needs an allocation" errors in Mutter
    ...
    add_blur(dash) {
        ...
        // Use a standard Meta.BackgroundGroup so Clutter/Mutter can allocate
        // it properly. Using FILL alignment ensures it does not contribute to
        // the preferred size of its parent (DashBox uses BinLayout which only
        // asks the primary child — the dash — for its preferred size).
        ...
        // Stop infinite relayout loop caused by BackgroundGroup fighting bin layouts
        background_group.queue_relayout = function() {};
