...
export const PanelVisibility = class PanelVisibility {
    ...
    connect_to_overview() {
        ...
        if (!this.settings.hidetopbar.COMPATIBILITY) {
            this.connections.connect(
                Main.overview, 'showing', _ => this.sync_overview_visibility()
            );
            this.connections.connect(
                Main.overview, 'hidden', _ => this.sync_overview_visibility()
            );
        } else {
            const appDisplay = this.get_app_display();
            if (!appDisplay) {
                this.schedule_overview_connect_retry();
                return;
            }

            this.connections.connect(
                appDisplay, 'show', _ => this.sync_overview_visibility()
            );
            this.connections.connect(
                appDisplay, 'hide', _ => this.sync_overview_visibility()
            );
            this.connections.connect(
                Main.overview, 'hidden', _ => this.sync_overview_visibility()
            );
    ...
    schedule_overview_connect_retry() {
        if (this._overview_connect_retry_scheduled)
            return;

        this._overview_connect_retry_scheduled = true;
        const retry = () => {
            this._overview_connect_retry_scheduled = false;
            if (!this.panel_blur.enabled || !this.settings.panel.UNBLUR_IN_OVERVIEW)
                return;

            if (this.get_app_display())
                this.connect_to_overview();
        };

        if (Main.layoutManager._startingUp) {
            this.connections.connect(Main.layoutManager, 'startup-complete', retry);
        } else {
            this.connections.connect(Main.overview, 'showing', retry);
        }
    }
    ...
    is_in_overview() {
        try {
            return !!(
                ...
                || (
                    this.panel_blur.is_main_panel_alive()
    ...
    sync_overview_visibility() {
        ...
        if (this.is_in_overview()) {
            this.panel_blur.hide();
            this.panel_blur.actors_list.forEach(actors =>
                this.set_should_override_panel(actors, false)
            );
            return;
        }

        this.panel_blur.show();
    ...
    connect_to_windows_and_overview() {
        ...
        this.schedule_startup_visibility_sync();
    }
    ...
    disconnect_from_windows_and_overview() {
        ...
        for (const actor of [
            Main.overview, Main.sessionMode, Main.layoutManager,
    ...
    update_visibility() {
        if (this.settings.panel.UNBLUR_IN_OVERVIEW && this.is_in_overview()) {
            this.panel_blur.actors_list.forEach(actors =>
                this.set_should_override_panel(actors, false)
            );
            return;
    ...
    set_should_override_panel(actors, should_override) {
        ...
        if (
            this.blur_settings.OVERRIDE_BACKGROUND
            &&
            should_override
        ) {
