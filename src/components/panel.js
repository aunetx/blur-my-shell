...
export const PanelBlur = class PanelBlur {
    ...
    blur_panel(panel) {
        ...
        if (this.settings.HACKS_LEVEL === 1) {
            ...
            const background = blur_surface.actor;
            const bg_manager = blur_surface.bg_manager;
            const dynamic_target = {
                queue_repaint: () => {
                    const active_pipeline = (
                        bg_manager && bg_manager._bms_pipeline
                    ) ? bg_manager._bms_pipeline : blur_surface.pipeline;

                    if (static_blur && active_pipeline?.effects) {
                        active_pipeline.effects.forEach(effect => {
                            effect?.queue_repaint?.();
                        });
                    } else if (!static_blur && active_pipeline) {
                        active_pipeline.repaint_effect?.();
                    }
                }
            };
            this.paint_signals.connect(background, dynamic_target);
        }
    ...
    destroy_blur(actors, panel_already_destroyed) {
        ...
        this.paint_signals.disconnect_all_for_actor(actors.blur_surface?.actor);
