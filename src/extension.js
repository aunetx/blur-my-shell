...
import { SettingsBindings } from './components/settings_bindings.js';
...
export default class BlurMyShell extends Extension {
    ...
    _update_clipped_redraws() {
        if (this._settings.DISABLE_CLIPPED_REDRAWS)
    ...
    _enable_components() {
        ...
        if (this._settings.panel.BLUR) {
            if (!this._panel_blur.enabled)
                ...
                this._panel_blur.refresh_after_startup();
        }
        ...
        if (this._settings.overview.BLUR) {
            if (!this._overview_blur.enabled)
                this._overview_blur.enable();
            else
    ...
    _connect_to_settings() {
        new SettingsBindings(this).connect();
    }
